...
TEST_CASES = [
    ...
    },
    {
        ...
        'threads_scope': [1, 2, 4, 8, 16]  # 按照AI推荐，主要测试8线程
    }
]
# ==============================================
    ...
    try:
        for case in TEST_CASES:
            ...
            for n in case['threads_scope']:
                ...
                # 为了数据准确，每个点测 3 次取平均（增加测试次数以提高准确性）
                samples = []
                for _ in range(3):
            ...
            print(f"   🏆 Optimal Concurrency: {best_thread} for {case['fixed_algo']} in {case['desc']}")
            # 输出AI推荐的并发数作为对比参考
            if case['id'] == 'Scenario_A_IoT':
                print(f"   🤖 AI Recommended Concurrency: 2 (optimized for weak network stability)")
            elif case['id'] == 'Scenario_B_Edge':
                print(f"   🤖 AI Recommended Concurrency: 4 (balanced performance/stability)")
            elif case['id'] == 'Scenario_C_Cloud':
                print(f"   🤖 AI Recommended Concurrency: 8 (optimized for high bandwidth)")
