...
def _run_cts_pipeline(
    ...
    try:
        ...
        # 🔥 Step 3+4: 解压并直接 pipe 给 docker load（不生成中间文件！）
        logger.info(f"[CTS] 解压并加载镜像（流式处理，无临时文件）...")
        ...
        with open(precompressed_path, 'rb') as f_in:
            ...
            # 启动 docker load 进程，接收解压后的数据
            p_load = subprocess.Popen(
                ["docker", "load"],
                stdin=p_decompress.stdout,
                stdout=subprocess.DEVNULL,
                stderr=subprocess.PIPE
            )
            
            # 关闭解压进程的 stdout，让它直接传递给 docker load
            p_decompress.stdout.close()
            
            # 等待两个进程完成
            _, err_decompress = p_decompress.communicate(timeout=600)
            _, err_load = p_load.wait(timeout=600)
        
        # 检查错误
        if p_decompress.returncode != 0:
            return False, 0.0, f"解压失败：{err_decompress.decode()[:200] if err_decompress else '未知错误'}"
        
        if p_load.returncode != 0:
            return False, 0.0, f"Docker Load 失败：{err_load.decode()[:200] if err_load else '未知错误'}"
        
        total_time = time.perf_counter() - total_start
        success = True
        
        logger.info(f"[CTS] 成功加载镜像，总耗时：{total_time:.2f}s")
        return success, total_time, ""

    except subprocess.TimeoutExpired:
        logger.error("❌ [CTS] 超时错误")
        return False, 0.0, "Timeout"
    except Exception as e:
        logger.error(f"❌ [CTS] 异常：{e}", exc_info=True)
        return False, 0.0, str(e)[:200]
